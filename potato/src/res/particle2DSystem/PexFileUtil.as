package potato.res.particle2DSystem
{
	...
	public class PexFileUtil
	{
		 ...
		 *  * （读取pex数据使用）<br>
		 */
		public static function loadPexFile(filePath:String,ps:ParticleSystem2D):void
		{
			...
			{
				if(PROPERTY[i]!=TEXTURE&&obj[PROPERTY[i]]!=null)
				{
			...
			else if(File.exists(filePath+"/../"+texturePath))// 尝试找filePath同一目录中的纹理（兼容pex）
			{
				ps.texture=new Texture(TextureData.createWithFile(filePath+"/../"+texturePath));
