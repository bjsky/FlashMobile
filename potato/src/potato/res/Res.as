...
//				log.debug("不能同时初始化2个");
		 ...
		 * 
		 */
		public static function getView(resId:String):View
		{
			var view:View;
			if(view == null)
			{
		 ...
		 * 
		 */
		public static function getViewSourcePath(resId:String):String
		{
		 ...
		 * 
		 */
		public function appendCfgBean(id:String,path:String,atlas:String,cache:int=0,type:int=-1,type1:int=0,type2:int=0,checkPlat:Boolean=false):void{
			...
			name_cfg[bean["id"]] = bean;
			if (bean.atlas) {
				readAtlasExt(bean.atlas, bean.id,checkPlat);
				var texture:Texture = new Texture(TextureData.createWithFile(bean.path));
				name_texture_weak[bean.id] = texture;
			}
		}
		
		private function readAtlasExt(atlas:String, id:String, checkPlat:Boolean=false):void
		{
			if(!checkPlat){			//编辑器跳过设备类型检查
				var xml:XML = new XML(File.read(atlas));
...
//				var path:String = resInfo.path;
		...
		public function addPriorLoad(arr:Array,isProgress:Boolean):void
		{
			...
			loader.addEventListener(HttpEvent.RES_LOAD_FAILED, onLoadField);
			_loadEndNum = 0;
			_priorLoaderList = arr;
			for(var i:int=0;i< _priorLoaderList.length;i++)
			{
				var name:String = _priorLoaderList[i];
				var bean:ResBean = getResBean(name);
				if(bean)
				{
					var md5:String = versionTest(bean.path);
					if(File.exists(bean.path) && md5==null)
...
//				}
		...
		private function onLoadField(e:HttpEvent):void
		{
		...
		private function isLoadEnd():void
		{
