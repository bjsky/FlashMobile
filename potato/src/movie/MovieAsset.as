package potato.movie
{
	...
	public class MovieAsset
	{
		...
		public static const CLIP_PREFIX:String="clip::";
...
//			}
		...
		private static function parseCfg(resPath:String):void {
			...
			for (var i:int = 1; i < row.length; i++)
			{
				...
				var clipBean:MovieBean=new MovieBean();
				for (var j:int = 0; j < title.length; j++)
				{
					if(title[j] == "frameArr")
					{
						...
						if (frameStr && frameStr.length > 0)
						{
							...
							clipBean.frameArr = frameStr.split(",");
							clipBean.frameArr.fixed = true;
						}
					} else {
						bean[title[j]] = col[j];
						clipBean[title[j]] = col[j];
					}
				}
				cfgList[bean.movieName] = bean;
				cfgList[CLIP_PREFIX+clipBean.movieName] =clipBean;
